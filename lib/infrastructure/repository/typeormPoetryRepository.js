...
class Repository {
    ...
    async findOne(queryOptions) {
        let queryBuilder
        if (_.isNull(this._transactionManager)) {
            queryBuilder = getRepository("Poetry").createQueryBuilder("poetry")
        }
        else {
            queryBuilder = this._transactionManager.createQueryBuilder("Poetry", "poetry")
        }
        queryBuilder.leftJoinAndMapOne("poetry.poetryType", "poetryTypes", "poetryType", "poetryType.id = poetry.poetryTypeID")
        const {
            poetryID
        } = queryOptions
        if (poetryID) {
