...
class Repository {
    ...
    async save(comment) {
        ...
        if (!commentEntity.id) {
            commentEntity.id = this.nextIdentify()
        }
    ...
    async findOne(queryOptions) {
        let queryBuilder
        if (_.isNull(this._transactionManager)) {
            queryBuilder = getRepository("Comment").createQueryBuilder("comment")
        }
        else {
            queryBuilder = this._transactionManager.createQueryBuilder("Comment", "comment")
        }
        queryBuilder.leftJoinAndMapOne("comment.user", "users", "user", "user.id = comment.userID")
