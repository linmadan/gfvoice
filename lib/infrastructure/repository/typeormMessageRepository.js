...
class Repository {
    ...
    async findOne(queryOptions) {
        let queryBuilder
        if (_.isNull(this._transactionManager)) {
            queryBuilder = getRepository("Message").createQueryBuilder("message")
        }
        else {
            queryBuilder = this._transactionManager.createQueryBuilder("Message", "message")
        }
        queryBuilder.leftJoinAndMapOne("message.toUser", "users", "user", "user.id = message.toUserID")
        const {
            messageID
        } = queryOptions
        if (messageID) {
