...
class Repository {
    ...
    async findOne(queryOptions) {
        let queryBuilder
        if (_.isNull(this._transactionManager)) {
            queryBuilder = getRepository("FocusRelation").createQueryBuilder("focusRelation")
        }
        else {
            queryBuilder = this._transactionManager.createQueryBuilder("FocusRelation", "focusRelation")
        }
        queryBuilder.leftJoinAndMapOne("focusRelation.follower", "users", "follower", "follower.id = focusRelation.followerID")
