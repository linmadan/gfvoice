            ...
            if (!_.isUndefined(rule[entityAttribute].appendType) && _.isUndefined(typeormEntity[entityAttribute])) {
                if (_.isArray(typeormEntity[entityAttribute])) {
                    ...
                    for (let appendType of typeormEntity[entityAttribute]) {
                        domainModel[entityAttribute].push(transformToDomainModel(appendType, rules[rule[entityAttribute].rule], rules[rule[entityAttribute].appendType]))
                    }
                } else {
                    domainModel[entityAttribute] = transformToDomainModel(typeormEntity[entityAttribute], rules[rule[entityAttribute].rule], rules[rule[entityAttribute].appendType])
                }
            } else {
