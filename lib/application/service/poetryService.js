...
class Service {
    ...
    async getPoetries(queryOptions) {
        const poetryRepository = createPoetryRepository()
        const {poetries, count} = await poetryRepository.find(queryOptions)
        const portryDAO = createPortryDAO()
        const rowDataPackets = await portryDAO.calculatePoetryUseNum(poetries.map(poetry => {
            return poetry.id
        }))
        const poetryUseNumMap = new Map()
        for (let rowDataPacket of rowDataPackets) {
            poetryUseNumMap.set(rowDataPacket.id, rowDataPacket.useNum)
        }
        return {
            count,
            list: poetries.map(poetry => {
                return {
                    ...poetry,
                    useNum: poetryUseNumMap.get(poetry.id)
                }
            })
        }
    }

    async listRecommendPoetry(queryOptions) {
        return []
    }
}
