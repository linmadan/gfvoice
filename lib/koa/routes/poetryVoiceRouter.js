...
router.get('/info/:poetryVoiceID', async (ctx, next) => {
    try {
        const {poetryVoiceID} = ctx.params
        const data = await ctx.poetryVoiceService.getPoetryVoice({
            poetryVoiceID,
            ...ctx.query
        })
        responseData(ctx, data)
...
router.get('/recommend', async (ctx, next) => {
    try {
        const data = await ctx.poetryVoiceService.listRecommendPoetryVoice({
            ...ctx.query
        })
        responseData(ctx, data)
    } catch (err) {
...
router.delete('/:poetryVoiceID/comments/:commentID', async (ctx, next) => {
    try {
        const {commentID} = ctx.params
        const data = await ctx.poetryVoiceService.removeComment(commentID)
        responseData(ctx, data)
    } catch (err) {
        responseError(ctx, err)
    }
...
router.delete('/:poetryVoiceID/replies/:replyID', async (ctx, next) => {
    try {
        const {replyID} = ctx.params
        const data = await ctx.poetryVoiceService.removeReply(replyID)
        responseData(ctx, data)
    } catch (err) {
        responseError(ctx, err)
    }
