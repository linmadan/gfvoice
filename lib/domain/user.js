...
class User {
    ...
    complainUser(user, reason) {
        return new Complain({
            type: constant.COMPLAIN_TYPE.USER,
            reason: reason,
            complainant: this.id,
            target: user.id
        })
    ...
    complainPoetryVoice(poetryVoice, reason) {
        return new Complain({
            type: constant.COMPLAIN_TYPE.POETRY_VOICE,
            reason: reason,
            complainant: this.id,
    ...
    complainComment(comment, reason) {
        return new Complain({
            type: constant.COMPLAIN_TYPE.COMMENT,
            reason: reason,
            complainant: this.id,
            target: comment.id
        })
    ...
    complainReply(reply, reason) {
        return new Complain({
            type: constant.COMPLAIN_TYPE.REPLY,
            reason: reason,
            complainant: this.id,
            target: reply.id
        })
